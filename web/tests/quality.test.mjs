...
test('unknown candidate states never echo provider content or imply activation',()=>{
  assert.equal(candidateLabel('<script>private token</script>'),"State not available");
  assert.match(candidateLabel('complete'),/observation/);
  assert.match(candidateLabel('not_configured'),/Waiting/);
...
test('missing kind model never invents a conversation or a safe classification',async()=>{
  ...
  assert.equal(mailKindLabel('unavailable'),"Undetermined type");
  assert.equal(mailKindLabel('<script>private</script>'),"Undetermined type");
