...
test('the console consumes the same versioned assessment cases as Rust and SMTP headers',()=>{
  for(const c of cases){
    ...
    assert.equal(classification(mail).label,input.decision?.source==='antivirus'?'Malware':{spam:'Spam',publicity:'Marketing',legitimate:'Legitimate',undetermined:'Needs review'}[e.category],c.name);
