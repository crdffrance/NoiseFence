...
type Metrics={messages:number;tp:number;fp:number;review:number;recall:number|null;fpr:number|null;precision:number|null;fpr_ci95:[number,number]|null;recall_ci95:[number,number]|null};
...
function MetricTable({report}:{report:Report}) {
  const rows=[['NoiseFence',report.baseline],['Rspamd',report.rspamd],['Candidate',report.candidate]] as const;
  return <div className="quality-table-scroll"><table className="quality-metrics"><caption>Human-labelled messages · missing decisions count as review</caption><thead><tr><th>Engine</th><th>Labelled</th><th>Recall</th><th>False positives</th><th>Precision</th><th>Review</th></tr></thead><tbody>
    {rows.filter(([,m])=>m).map(([name,m])=><tr key={name}><th>{name}</th><td>{m!.messages}</td><td>{percent(m!.recall)}{m!.recall_ci95&&<small>95% CI {m!.recall_ci95.map(percent).join('–')}</small>}</td><td>{m!.fp} · {percent(m!.fpr)}{m!.fpr_ci95&&<small>95% CI {m!.fpr_ci95.map(percent).join('–')}</small>}</td><td>{percent(m!.precision)}</td><td>{m!.review}</td></tr>)}
  </tbody></table></div>;
}
...
export function QualityWorkbench({user,batch,purpose,refresh}:{user:User;batch:string;purpose:DatasetPurpose;refresh:number}) {
  ...
  return <section className="panel quality-workbench"><p className="eyebrow">CALIBRATION WORKBENCH</p><h2>Measure before changing decisions</h2>
    ...
    {state?.jobs.map(j=><details className="quality-job" key={j.id}><summary><strong>{j.operation==='train'?'Training':j.operation==='evaluate'?'Candidate evaluation':'Engine comparison'}</strong><span>{j.status.replaceAll('_',' ')} · {new Date(j.created*1000).toLocaleString('en-GB')}</span></summary>
      ...
      {j.report&&<><MetricTable report={j.report}/>{j.report.coverage&&<p>{j.report.coverage.labelled??j.report.coverage.usable??0} usable or labelled observations · {j.report.coverage.unlabelled_or_uncertain??0} unlabelled or uncertain · {j.report.coverage.deleted_or_no_longer_authorized??0} missing from the original draw.</p>}
      {j.report.campaigns&&<details><summary>Campaign-level comparison</summary><MetricTable report={j.report.campaigns}/><p className="muted small">One representative per campaign; conflicting labels are excluded and reported.</p></details>}
