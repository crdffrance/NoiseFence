...
export function QualityWorkbench({user,batch,purpose,refresh}:{user:User;batch:string;purpose:DatasetPurpose;refresh:number}) {
  ...
  return <section className="panel quality-workbench"><p className="eyebrow">CALIBRATION WORKBENCH</p><h2>Measure before changing decisions</h2>
    ...
    <div className="quality-controls"><Button disabled={busy||!batch} variant="outline" onClick={()=>action('/quality/jobs',{batch,operation:'compare',candidate:null})}>{purpose==='holdout'?'Compare (uses holdout)':'Compare this sample'}</Button>
    ...
    {purpose==='holdout'&&<p className="notice">The first export consumes this holdout for freshness tracking, including comparisons and failed attempts. Evaluate the frozen candidate before other experiments; use a new future sample for another independent test.</p>}
