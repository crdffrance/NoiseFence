...
const details:Record<string,string>={unsafe_url:'URL hors du périmètre autorisé',forbidden_address:'Adresse réseau interdite',dns:'Résolution DNS indisponible',network:'Connexion indisponible',http_status:'Réponse HTTP non exploitable',invalid_redirect:'Redirection ambiguë',loop:'Boucle de redirection',hop_limit:'Nombre de redirections atteint',body_limit:'Page trop volumineuse',encoding:'Encodage non pris en charge',client_script:'Destination dépendante de JavaScript',deadline:'Délai de contrôle atteint',busy:'Capacité de contrôle atteinte'};
export function detailLabel(value:string){return details[value] || 'Limite non reconnue';}
const diagnostics:Record<string,string>={analysis_incomplete:'Analyse incomplète',extraction_incomplete:'Extraction partielle',authentication_unavailable:'Authentification indisponible',provider_incomplete:'Réputation externe incomplète',redirect_incomplete:'Destination de lien non résolue',decision_disagreement:'Désaccord entre les avis',corroboration_absent:'Preuves de confirmation absentes'};
export function diagnosticLabel(value:string){return diagnostics[value] || 'Contexte non reconnu';}
