import { comparisonLabel, comparisonPoints, agreementRate, type RspamdReport, type ComparisonSummary } from './rspamd-format';
...
export function RspamdComparison({ report, mail, onRefresh }: { report?: RspamdReport | null; mail: ScoreInput; onRefresh: () => void }) {
  ...
  return <section className="panel rspamd-comparison" aria-label="Rspamd engine comparison">
    <div className="rspamd-heading"><div><p className="eyebrow">INDEPENDENT COMPARISON</p><h2>Rspamd</h2></div>
      <span className={`status ${report?.comparison === 'disagreement' && report.status === 'complete' ? 'review' : ''}`}>{comparisonLabel(report)}</span>
      {report?.status === 'pending' && <button type="button" onClick={onRefresh}>Refresh comparison</button>}
    </div>
    <p className="muted">The original message is checked separately. Rspamd never changes NoiseFence decisions or delivery.</p>
    ...
    </> : <p>No Rspamd score is available. NoiseFence analysis remains independent. Comparisons start on new accepted SMTP messages; older messages are not replayed.</p>}
