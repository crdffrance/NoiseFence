//! Durable selective RCPT admission. No message acceptance, delivery, or challenge email.
//!
...
pub mod runtime;
pub const VERSION: &str = "smtp-admission-2";
...
impl Settings {
    pub fn validate(&self) -> Result<()> {
            ...
            self.allow_networks.len() <= 128,
            "At most 128 admission exceptions"
        );
        for network in &self.allow_networks {
            runtime::network(network)?;
        }
        ensure!(
            self.rate_per_minute <= 60000 && (1..=10000).contains(&self.rate_burst),
            "Invalid SMTP rate limit"
        );
        ensure!(
...
impl Decision {
    ...
    pub fn smtp_reply(&self) -> Option<&'static str> {
        if !self.enforced {
            None
        } else if self.status == Status::RateLimited {
            Some("451 4.7.1 Sender rate limit; please retry later\r\n")
        } else {
            Some(GREYLIST_REPLY)
        }
    }
...
pub struct Admission {
    settings: Settings,
    sleepers: Arc<Capacity>,
}
impl Admission {
    pub fn new(settings: Settings) -> Result<Self> {
        settings.validate()?;
        Ok(Self {
            sleepers: Capacity::new(settings.tarpit_max_concurrent),
            settings,
        })
    }
    ...
    /// Prepare policy without activating limits; only the committed revision
    /// calls activate(), so old in-flight sessions cannot restore an old limit.
    ...
    pub fn activate(&self) {
        self.sleepers.set_limit(self.settings.tarpit_max_concurrent);
    }
    ...
    pub fn check(
        ...
        let decision = if let Some((id, first_seen, eligible, passed)) = row {
            if now < first_seen || passed.is_some_and(|at| now < at) {
                self.decision(Status::ClockSkew, false, None)
    ...
    /// State failures are visible but fail open in every mode, without a sleep.
    pub fn unavailable(&self) -> Decision {
        if self.settings.enabled {
            self.decision(Status::Unavailable, false, None)
        } else {
    ...
    pub(crate) fn decision(
        ...
        Decision {
            ...
            would_defer,
            retry_after_seconds,
            candidate_delay_ms: if matches!(
                status,
                Status::FirstSeen | Status::TooSoon | Status::RateLimited
            ) {
